...
int mkfifo_s(int flags) {
    ...
    sys_err(mkfifo(FIFO_NAME, S_IRUSR | S_IWUSR), MKFIFO_ERR);
...
int main(int argc, char** argv) {
    ...
    int fd = mkfifo_s(O_WRONLY);
    sys_err(dup2(fd, STDOUT_FILENO), DUP_ERR);    
    close(fd);

    const char* file = argv[1];
    int err = execlp("cat", "cat", file, NULL);
