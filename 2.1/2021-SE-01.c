...
int main(int argc, char** argv) {
    ...
    while ( (bytes_read = read(input, buf, sizeof(buf))) > 0 ) {
        ...
        for (size_t i = 0; i < bytes_read_positive; i++) {
            ...
            uint8_t first = (res >> 8) & 0xFF;
            uint8_t last = res & 0xFF;
            sys_err(write(output, &first, sizeof(uint8_t)), WRITE_ERR, WRITE_MSG);
            sys_err(write(output, &last, sizeof(uint8_t)), WRITE_ERR, WRITE_MSG);
        }
    }
